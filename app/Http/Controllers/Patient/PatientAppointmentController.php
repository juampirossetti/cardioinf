...
class PatientAppointmentController extends AppBaseController
{
    ...
    {   
        ...
        if($input['owner'] != 'me'){
            ...
            if($request->has('patient_primary_phone')){
                $input['patient_primary_phone'] = $request->get('patient_primary_phone');
            } else {
                $error = true;
            }
            if($error){
                Flash::error('El DNI, teléfono, nombre y apellido no pueden estar vacios. Por favor complételos y solicite su turno nuevamente');
                return redirect(route('patient.appointments.index')); 
            }
        }
