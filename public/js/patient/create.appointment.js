$(document).ready(function () {
  ...
  function updateNombrePaciente() {
    ...
    $('input[name=patient_dni]').val($('#patient_dni').val());
    $('input[name=patient_primary_phone]').val($('#patient_primary_phone').val());
  }
  ...
  var showOrHideCarousel = function () {
    ...
    if (
      !insuranceEnabled ||
      $('#medical_study_id option:selected').val() == 'null' ||
  ...
  var populateAppointmentsList = function (data) {
    ...
    for (var key in data) {
      ...
      if (data[key].length == 0) {
        //si no hay turno mostrar mensaje
        $('<div class="no-appointments">Ningún turno disponible para este día</div>').appendTo(scrollable_menu);
      } else {
        //si hay turnos los cargo
        for (var i = 0; i < data[key].length; i++) {
          $('<li class="appointment-item"><a href="javascript:void(0);">' + data[key][i] + '</a></li>').appendTo(
            scrollable_menu
          );
        }
      }
