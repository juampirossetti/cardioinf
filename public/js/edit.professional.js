$(document).ready(function() {
    ...
    $('.select-all').on('click', function(event) { 
        $('#insurances_select option').prop('selected', true);
        $('#insurances_select option').eq(0).prop('selected', false);
    });
    ...
    var updateUser = function(user_email, professional_id){
        
        $.ajax({
            url: '/api/user/generateProfessionalPassword',
            data: { 
